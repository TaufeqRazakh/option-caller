...
--If I could repace Either with Data.Aeson.Parser maybe I can get more speed
                            ...
                            (symbol_:toTime) -> case toTime of
                              ...
                              (expiryDay_:expiryTime_:toStrike) -> case toStrike of
                                ...
                                (strike_ :toPrice) -> case toPrice of
                                  [] -> Left "insufficient input params"
                                  (price_:rest) ->
                                    Right Option { Types.Option.symbol = symbol_
  ...
  toStock inputText = let inputSet = T.words inputText
                      in case inputSet of
                           ...
                           (symbol_:toValue) -> case toValue of
                             [] -> Left "insufficient input params"
                             (value_:rest) -> 
                                 Right Stock { Types.Stock.symbol = symbol_
                                             , Types.Stock.value  = read (T.unpack value_)
                                             , Types.Stock.price  = read $ T.unpack $ justifyRight 1 '0' (T.unlines rest)
                                             }
